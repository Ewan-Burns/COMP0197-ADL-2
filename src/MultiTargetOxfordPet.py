...
class MultiTargetOxfordPet(Dataset):
    def __init__(self, use_breed=False, random_hflip=True, random_vflip=True, seed = 42):
        super().__init__()
        self.seed = seed
    ...
    def __getitem__(self, item):
        ...
        if item_rng.random() < 0.5 and self.random_hflip:
            image = F.hflip(image)
            mask = F.hflip(mask)
    
        if item_rng.random() < 0.5 and self.random_vflip:
            image = F.vflip(image)
            mask = F.vflip(mask)
