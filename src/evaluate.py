...
def load_model(model_type, model_path, num_classes, device):
    ...
    if model_type == "deeplabv3":
        ...
        model = WSSDeepLabV3(num_classes=num_classes)
    elif model_type == "resnet_sec":
        model = MultiHeadResNet(num_classes=num_classes)
    else:
        raise ValueError(
            f"Unknown model type: {model_type}. Choose 'unet', 'deeplabv3', or 'resnet_sec'."
        )
...
def evaluate(model, dataloader, device, num_classes):
    ...
    with torch.no_grad():
        ...
        for images, masks, _ in bar:
            ...
            elif isinstance(model, WSSDeepLabV3):  # heuristic for DeepLabV3
                outputs = model(images)  # N, C, H, W
            else:
                raise TypeError(
                    f"Model output handling not defined for type: {type(model)}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["unet", "deeplabv3", "resnet_sec"],
